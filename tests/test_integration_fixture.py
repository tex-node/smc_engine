...
def build_fixture():
    ...
    d1 = frame([
        [f"D{i}", 100, 101, 99, 100] for i in range(20)
    ])
    ...
    h4_rows = [[f"H{i}", 100, 101, 99, 100] for i in range(30)]
    # Confirmed sell-side swing low.
    h4_rows[10] = ["H10", 100, 101, 95, 99]
    h4_rows[11] = ["H11", 99, 100, 97, 98]
    h4_rows[12] = ["H12", 98, 100, 96, 99]
    # Sweep below H10 and close back above.
    h4_rows[15] = ["H15", 99, 100, 94, 99]
    # Bullish structural break after sweep.
    h4_rows[16] = ["H16", 99, 104, 98, 103]
    h4 = frame(h4_rows)

    m15_rows = [[f"M{i}", 100, 101, 99, 100] for i in range(100)]
    # Post-CSD bearish candle becomes bullish OB.
    m15_rows[60] = ["M60", 100, 101, 98, 99]
    # Displacement.
    m15_rows[61] = ["M61", 99, 110, 98, 109]
    # Later swing/target.
    m15_rows[70] = ["M70", 109, 115, 108, 114]
    m15_rows[71] = ["M71", 114, 114, 111, 112]
    # Return to OB.
    m15_rows[80] = ["M80", 112, 113, 99, 101]
    m15 = frame(m15_rows)
...
def test_end_to_end_fixture_produces_or_rejects_candidate_deterministically():
    ...
    first = analyzer.analyze_at(d1, h4, m15, as_of="M99")
